...
def filter_not_already_used_holds(boulder: list, beta: list) -> list:
    ...
    return [hold for hold in boulder if not (hold in used_holds_left and hold in used_holds_right)]  # Return only the holds not used in the beta
...
def find_beta_next_hold(boulder: list, beta_list: list, holds_data: dict, span: int=180) -> list[list[str]]:
    ...
    print(f"Iteration: {len(beta_list)} betas to process")
    next_beta_list = []
    ...
    for beta in beta_list:
        ...
        for next_hold in next_holds:
            ...
            for mouv, other_mouv in [(last_mouv, previous_other_hand), (previous_other_hand, last_mouv)]:
                ...
                next_hold_pos = commons.hold_name_to_pos(next_hold)
                hold_pos = commons.hold_name_to_pos(hold)
                if (
                    next_mouv in beta
                    or commons.get_distance(hold, next_hold) > span - 22
                    ...
                    not is_crossing(next_mouv, mouv)
                    or (is_crossing(next_mouv, mouv) and commons.get_distance(hold, next_hold) < (span - 22) / 2 and abs(hold_pos[1] - next_hold_pos[1]) < 3 and "S" not in orientation)  # If the next hold is close enough to the last hold and is a crossing move and the orientation is not "south" (undercling)
...
def evaluate_beta_difficulty(beta: list, holds_data: dict) -> dict:
    ...
    for index, mouv in enumerate(beta):
        ...
        _, previous_other_hand = find_previous_other_hand_mouv(beta[:index+1])  # Find the previous move with the other hand
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python beta.py <span> <boulder>")
        sys.exit(1)
