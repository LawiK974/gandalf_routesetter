...
def get_distance(a: str, b: str) -> float:
    ...
    a_row, a_col = hold_name_to_pos(a)
...
def get_hold_difficulty(hand: str, hold_data: dict) -> float:
    ...
    orientation_point = {
        "N": 0,
        "S": 4,
        "NE": 1 if hand == "L" else 0,
        "NW": 1 if hand == "R" else 0,
        "E": 10 if hand == "L" else 5,
        "W": 10 if hand == "R" else 5,
...
def load_holds_data(version="2019"):
    ...
    for hold in dataset:
        ...
        new_dataset["L" + hold] = {**dataset[hold], "difficulty": get_hold_difficulty("L", dataset[hold])}
